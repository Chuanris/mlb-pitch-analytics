...
| Build a versioned MLB source audit | `.\.venv\Scripts\python.exe -m src.build_mlb_source_audit --start-date YYYY-MM-DD --end-date YYYY-MM-DD --output evidence/mlb-source-audit/<range>.json` | Read-only DuckDB plus live official requests; writes a compact manifest even when mismatches produce a nonzero exit |
| Check dashboard payload budgets | `node scripts/check_dashboard_payload.mjs` after building the dashboard | Read-only; checks snapshot, built HTML, deterministic gzip-9, largest-query and row-count limits from `config/dashboard_performance.json` |
...
1. Inspect `git status --short` and `git diff`. Keep local lessons, `.env`, raw data, databases, model files, outputs, logs and browser profiles out of the commit. Commit authored source, tests, the reviewed public snapshot, and any compact versioned evidence manifest supporting a published claim together.
2. Run Python tests from the root: `.\.venv\Scripts\python.exe -m unittest discover -s tests -p "test_*.py"`. Run `node --test dashboard/tests/mlb-pitch-analysis.test.mjs dashboard/tests/mlb-hitter-analysis.test.mjs dashboard/tests/dashboard-payload-budget.test.mjs dashboard/tests/snapshot-integrity.test.mjs` for the MLB aggregation, hitter decision helpers, payload budget, and public snapshot contracts.
...
3. In `dashboard/`, run `npm.cmd ci` and `npm.cmd run build`. From the root, run `node scripts/check_dashboard_payload.mjs`, then `node scripts/check_compare.mjs prepublish`. Chrome must be installed for the browser check; `CHROME_PATH` overrides its location.
4. Commit the reviewed files and push to `main`. Dashboard changes, either dashboard-check script, or the dashboard performance budget trigger the Pages workflow; documentation-only changes do not require rebuilding the site. The workflow can also be dispatched manually.
...
| Payload budget fails | Read the named metric and limit, then inspect query row count and serialized bytes. Change a budget only for reviewed scope growth; do not raise it merely to bypass a regression. |
...
| 建立版本化 MLB 來源稽核 | `.\.venv\Scripts\python.exe -m src.build_mlb_source_audit --start-date YYYY-MM-DD --end-date YYYY-MM-DD --output evidence/mlb-source-audit/<range>.json` | 唯讀 DuckDB 加上官方即時請求；即使 mismatch 回傳非零，也會寫出精簡 manifest |
| 檢查 dashboard payload 預算 | dashboard build 後執行 `node scripts/check_dashboard_payload.mjs` | 唯讀；依 `config/dashboard_performance.json` 檢查 snapshot、built HTML、deterministic gzip-9、最大 query 與 row-count 上限 |
...
1. 檢查 `git status --short` 與 `git diff`。個人 lesson、`.env`、原始資料、資料庫、模型、outputs、日誌與瀏覽器設定檔保持在本機；功能原始碼、測試、已檢查的公開快照，以及支撐公開說法的精簡版本化 evidence manifest 一併提交。
2. 在根目錄執行 Python 測試：`.\.venv\Scripts\python.exe -m unittest discover -s tests -p "test_*.py"`；執行 `node --test dashboard/tests/mlb-pitch-analysis.test.mjs dashboard/tests/mlb-hitter-analysis.test.mjs dashboard/tests/snapshot-integrity.test.mjs` 驗證 MLB 聚合、打者決策邏輯與公開快照契約。
...
3. 在 `dashboard/` 執行 `npm.cmd ci` 與 `npm.cmd run build`，回根目錄先執行 `node scripts/check_dashboard_payload.mjs`，再執行 `node scripts/check_compare.mjs prepublish`。Browser check 需安裝 Chrome；可透過 `CHROME_PATH` 指定安裝位置。
...
| Payload budget 失敗 | 先查看錯誤指出的 metric 與 limit，再檢查 query row count 與 serialized bytes。只有已審查的 scope 增長才能調整 budget，不可單純提高上限以繞過 regression。 |
