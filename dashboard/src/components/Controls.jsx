  ...
  return (
    <section ref={filterBarRef} className={sticky ? "filters filter-bar" : "filters"} aria-label={ariaLabel}>
      {filters.map((filter) => {
        ...
        const formatTemporalChoice = temporal ? (choice) => {
        ...
        const filterChoiceIcons = choiceIcons[filter.id];
        const formatChoice = filterChoiceIcons ? (choice) => {
          const text = formatTemporalChoice?.(choice) ?? choice;
          const icon = filterChoiceIcons[choice];
          return icon ? <span className="mlb-team-choice">
            <img src={icon} alt="" aria-hidden="true" />
            <span>{text}</span>
          </span> : text;
        } : formatTemporalChoice;
